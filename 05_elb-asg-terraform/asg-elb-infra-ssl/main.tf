...
# ACM 인증서용 RSA 프라이빗 키 자동 생성
resource "tls_private_key" "acm" {
  algorithm = "RSA"
  rsa_bits  = 2048
}

# 자체 서명 인증서(Self-Signed Certificate) 자동 생성 - 실습/테스트 환경용
resource "tls_self_signed_cert" "acm" {
  private_key_pem = tls_private_key.acm.private_key_pem

  subject {
    common_name  = "example.com" # 인증서에 표시될 도메인 이름
    organization = "Example Org" # 인증서에 표시될 조직명
  }

  validity_period_hours = 8760 # 인증서 유효 기간 (1년 = 8760시간)

  allowed_uses = [
    "key_encipherment",  # 키 암호화 허용 (TLS 핸드셰이크에 필요)
    "digital_signature", # 디지털 서명 허용
    "server_auth",       # 서버 인증 용도로 사용 허용
  ]
}

# ACM 인증서 리소스: Terraform이 자동 생성한 인증서를 ACM에 업로드
resource "aws_acm_certificate" "example" {
  private_key      = tls_private_key.acm.private_key_pem # 자동 생성된 프라이빗 키
  certificate_body = tls_self_signed_cert.acm.cert_pem   # 자동 생성된 자체 서명 인증서
}
...
resource "aws_security_group" "alb" {
  ...
  ingress {
    description = "Allow HTTP from internet" # HTTP → HTTPS 리다이렉트용
    from_port   = 80
    to_port     = 80
    protocol    = "tcp"
  ...
  ingress {
    description = "Allow HTTPS from internet" # 인터넷에서 오는 HTTPS 트래픽 허용
    from_port   = 443
    to_port     = 443
    protocol    = "tcp"
    cidr_blocks = ["0.0.0.0/0"]
...
# 애플리케이션 로드 밸런서(ALB) - 외부 HTTPS 트래픽을 EC2 인스턴스로 분산
...
# HTTPS 리스너 - 포트 443으로 들어오는 요청을 타겟 그룹으로 전달
...
# HTTP → HTTPS 리다이렉션 리스너 - 포트 80으로 오는 요청을 443으로 자동 전환
...
resource "aws_lb_target_group" "example" {
  name        = "example-tg-${random_integer.example.result}" # 이름 충돌 방지를 위한 랜덤 suffix
  port        = 80
  protocol    = "HTTP"
