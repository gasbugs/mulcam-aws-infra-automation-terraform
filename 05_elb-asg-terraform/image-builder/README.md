...
[결과 AMI: spring-boot-app-ami-YYYYMMDDHHMMSS]
  ...
  - systemd 서비스 자동 시작 설정됨 (ec2-user 권한, HTTP 8080)
...
> 4. CodeCommit에 push
>
> Spring Boot 앱은 일반 사용자(`ec2-user`) 권한으로 실행되므로 기본 포트는 8080입니다.
> `../packer-for-javaspring/src/main/resources/application.properties`와 Image Builder systemd 서비스가 모두 8080을 사용합니다.
