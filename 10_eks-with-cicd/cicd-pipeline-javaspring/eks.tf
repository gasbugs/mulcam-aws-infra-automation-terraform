...
# EKS 관리형 노드 그룹 — ArgoCD와 Java Spring 앱이 실행될 워커 노드
...
# ArgoCD가 javaspring-apps CodeCommit 저장소를 SSH로 읽을 수 있도록 자동 설정
...
# ArgoCD Application 리소스 — javaspring-apps 저장소와 EKS 클러스터를 연결
# javaspring-apps에 매니페스트가 push되면 ArgoCD가 자동으로 EKS에 배포
  ...
  yaml_body = yamlencode({
    ...
    spec = {
      ...
      source = {
        ...
        targetRevision = "main"
        path           = "."  # javaspring-apps 루트에 매니페스트 위치
      }
      destination = {
        server    = "https://kubernetes.default.svc"
        namespace = "javaspring-app"
      }
      syncPolicy = {
        automated = {
        ...
        syncOptions = ["CreateNamespace=true"]  # 대상 네임스페이스(javaspring-app)가 없으면 자동 생성
      }
    }
  })
