...
###################################################################
# CodeCommit 저장소 생성
# javaspring: Java Spring Boot 앱 소스 코드를 저장 (CodePipeline 소스)
...
# javaspring-apps: ArgoCD가 읽어갈 Kubernetes 매니페스트 저장소
...
###################################################################
# CodeBuild 프로젝트 구성
# Java Spring Boot 앱을 멀티 스테이지 Docker 빌드로 컴파일하여 ECR에 푸시
# buildspec.yml이 javaspring 레포 루트에 있어야 함
resource "aws_codebuild_project" "this_ci" {
  ...
  build_timeout = "20" # Java 컴파일 + 테스트 포함으로 Flask보다 빌드 시간이 더 소요됨
  ...
  source {
    type      = "CODEPIPELINE"
    buildspec = "buildspec.yml" # javaspring 레포 루트에 있는 buildspec.yml 사용
  }
}
...
resource "aws_cloudwatch_event_rule" "codecommit_trigger" {
  ...
  event_pattern = jsonencode({
    ...
    resources   = [aws_codecommit_repository.javaspring.arn]
    detail = {
...
resource "aws_codepipeline" "this" {
  ...
  # 두 번째 단계: CodeBuild로 Java Spring Boot Docker 이미지 빌드 및 ECR 푸시
  stage {
    name = "Build"
