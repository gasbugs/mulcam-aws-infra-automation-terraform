...
1. **S3 버킷**: 파이프라인 아티팩트 저장용 (삭제 시 모든 객체도 함께 삭제)
2. **CloudWatch 로그 그룹**: CodeBuild 빌드 로그 저장용
3. **ECR 리포지토리**: Docker 이미지 저장소
4. **IAM 역할 및 정책**: CodePipeline, CodeBuild, Events 서비스용
5. **CodeBuild 프로젝트**: VPC 설정 및 보안 그룹 포함
6. **CodeStar Connections**: GitHub 연동 (수동 승인 필요)
7. **CodePipeline**: Source(GitHub) → Build(CodeBuild) 구성
8. **GitHub Webhook**: main 브랜치 push 시 파이프라인 자동 트리거
...
**CodeStar Connection 활성화** (수동 작업 필요):
1. AWS Console → CodePipeline → 설정 → Connections
2. Pending 상태인 연결 선택 → "Update pending connection" 클릭
3. GitHub 계정 인증 완료
...
2. Settings → Repositories → netflux-deploy GitHub 연결
3. Applications → New App → netflux-deploy 구성
