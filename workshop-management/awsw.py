...
#   nuke      aws-nuke 를 사용한 계정 전체 리소스 일괄 삭제 (audit+clean 대체 예정)
#   vpc       Default VPC + 서브넷 복원 (nuke 후 사용)
#   cost      전일 비용 리포트
...
from commands import setup, teardown, audit, clean, nuke, vpc, cost, check, tag, admin, status, creds, workflow, cloudfront
...
cli.add_command(nuke.cmd,        name="nuke")
cli.add_command(vpc.cmd,         name="vpc")
